...
import { useAccountantNotifications } from '@/hooks/accountant/useAccountantNotifications';
...
export default function AccountantNotificationsPage() {
  ...
  if (isLoading) {
    return (
      <PageContainer>
        <SectionHeader title="Finance Alerts" description="Loading portal alerts..." />
  ...
  if (isError) {
    return (
      <PageContainer>
        <SectionHeader title="Finance Alerts" description="Accountant alerts inbox." />
        <ErrorState message="Failed to fetch accountant notifications. Please verify network connectivity." onRetry={refetch} />
      </PageContainer>
    );
  }
  ...
  return (
    <PageContainer>
      <SectionHeader
        ...
        description="Monitor accountant-specific alerts, student payment notifications, outstanding invoice summaries, and payroll statuses."
        action={
          unreadCount > 0 && (
            <Button
              variant="outline"
      ...
      {filteredNotifications.length === 0 ? (
        <EmptyState
          ...
          description={
            statusFilter === 'unread'
              ? 'Excellent! You have read all notifications in your financial portal.'
              : 'You do not have any notices in your financial alert center.'
          }
      ...
      <ConfirmDialog
        ...
        description="Are you sure you want to dismiss all pending finance alerts? This action cannot be undone."
        onConfirm={async () => {
          await markAllAsRead();
        }}
      />
